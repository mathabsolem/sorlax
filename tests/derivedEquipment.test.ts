...
describe('Prozentuale Beitraege', () => {
  ...
  it('addiert zwei Teile mit je 8 Prozent meleeBonus zu 0.16, nicht zu 0.1664', () => {
    ...
    wear(state, content, 'weapon', 'weapon_rig_light', [{ affixId: 'pre_brutal', value: 8 }]);
    wear(state, content, 'gloves', 'gloves_wrap', [{ affixId: 'pre_brutal', value: 8 }]);
  ...
  it('addiert prozentuale Groessenwerte ebenfalls, statt sie zu multiplizieren', () => {
    ...
    wear(state, custom, 'suit', 'suit_liner', [{ affixId: 'pct_life', value: 10 }]);
    wear(state, custom, 'guard', 'guard_buckler', [{ affixId: 'pct_life', value: 10 }]);
...
describe('Attributsaffixe', () => {
  ...
  it('suf_of_might mit Wert 4 hebt strength und damit meleeBonus', () => {
    ...
    wear(state, content, 'gloves', 'gloves_wrap', [{ affixId: 'suf_of_might', value: 4 }]);
  ...
  it('vitality aus einem Affix hebt maxHealth ueber die Attributsformel', () => {
    ...
    wear(state, content, 'helmet', 'helmet_cap', [{ affixId: 'suf_of_focus', value: 6 }]);
...
describe('Resistenzen', () => {
  ...
  it('zaehlt zwei Teile mit je 20 auf normal zu 40 und auf nightmare zu minus 60', () => {
    const { state, content } = setup();
    wear(state, content, 'suit', 'suit_liner', [{ affixId: 'suf_of_embers', value: 20 }]);
    wear(state, content, 'guard', 'guard_buckler', [{ affixId: 'suf_of_embers', value: 20 }]);
  ...
  it('deckelt fuenf Teile mit je 20 Feuerresistenz auf 75', () => {
    ...
    const bases = ['suit_liner', 'guard_buckler', 'amulet_tag', 'helmet_cap', 'belt_strap'];
...
describe('Weitere Werte aus der Ausruestung', () => {
  it('hebt Sichtweite, freie Aktion und Munitionsersparnis', () => {
    const { state, content } = setup();
    wear(state, content, 'helmet', 'helmet_cap', [{ affixId: 'suf_of_the_lamp', value: 3 }]);
    wear(state, content, 'boots', 'boots_tread', [{ affixId: 'suf_of_haste', value: 8 }]);
    wear(state, content, 'belt', 'belt_strap', [{ affixId: 'suf_of_thrift', value: 12 }]);
  ...
  it('rechnet Ruestung und Genauigkeit aus den Grundwerten der Teile', () => {
    const { state, content } = setup();
    wear(state, content, 'suit', 'suit_liner', [{ affixId: 'pre_plated', value: 4 }]);
    wear(state, content, 'gloves', 'gloves_wrap', [{ affixId: 'pre_honed', value: 5 }]);

    const stats = statsOf(state, content);
    expect(stats.armor).toBe(6);
...
describe('Ausgeruestete Gegner', () => {
  it('nimmt die Affixe der getragenen Teile in die Gegnerwerte auf', () => {
    ...
    entity.equipment = {
      suit: craft(state, content, 'suit_liner', [{ affixId: 'pre_sturdy', value: 14 }]),
    };
  ...
  it('bildet suf_of_the_lamp bei Gegnern auf die Aggroreichweite ab', () => {
    ...
    entity.equipment = {
      helmet: craft(state, content, 'helmet_cap', [{ affixId: 'suf_of_the_lamp', value: 3 }]),
    };
